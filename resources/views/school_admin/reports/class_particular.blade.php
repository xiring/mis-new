...
@section('content')
        ...
        <div class="page-breadcrumb">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ $page }} : {{ @$fee->name }}</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body table-responsive">
            <form method="get" action="{{ route('report.class.particular') }}">
                <div class="form-group">
                    <label class="col-form-label">Choose Class</label>
                    <select class="form-check-inline col-md-12 select2" name="class_id" onchange="getClassFees(this.value);">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2" name="fee_category_id" id="fee_holder">
...
@section('customScript')
    ...
    <script>
        function getClassFees(class_id) {
            if (class_id !== '') {
                $.ajax({
                    url: '{{ url('/school/dashboard/class/fees') }}'+'/'+ class_id ,
                    success: function(response)
                    {
                        jQuery('#fee_holder').html(response);
                    }
                });
            }
        }
