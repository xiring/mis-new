            ...
            <div class="card-body table-responsive">
                ...
                {{ Form::open(['route' => 'marks.manage.update' , 'files' => true]) }}
                    ...
                    <table class="table table-bordered" id="exam">
                        ...
                        <tbody>
                            @foreach($enrolls_asc as $enroll_asc)
                                @php
                                    $mark = App\Models\ExamMark::where('student_id', $enroll_asc->student->id)->where('school_id', $school->id)->where('year', $school->detail->running_session)->where('class_id', $class->id)->where('section_id', $section->id)->Where('exam_id', $exam->id)->where('subject_id', $subject->id)->first();
                                @endphp
                                <tr>
                                    <td>{{ @$enroll_asc->roll_number }}</td>
                                    <td>
                                        <input type="hidden" name="student_ids[]" value="{{ @$mark->student_id }}">
                                        {{ @$enroll_asc->student->user->name }}
                                    </td>
                                    <td>
