...
<div class="main-wrapper" id="invoice_print1" style="page-break-after: always;">
		        ...
		        <div  class="student-details clear">
		            ...
		            <div class="first-term">
		                                        ...
		                                        @if($mark->subject->type == 'is_none')
		                                            ...
		                                            @else
		                                                @php
		                                                    ...
		                                                    echo  '<b class=""style="">'.$marks_obtained.'</b>';
		                                                @endphp
		                                            @endif
		                                        @endif
		                                    </td>
		                ...
		                <table class="one-gray">
		                    <tr >
		                    	@php
		                    		$attend = App\Models\ManualAttentace::where('exam_id', $exam->id)->where('class_id', $class->id)->where('year', $school->detail->running_session)->where('student_id', $student->id)->first();
		                    	@endphp
		                ...
		                <div style="width: 100%; margin-bottom: 10px;">
		                    <div style="float: left;width: 55%; font-size: 12px;">
		                        ...
		                        @if(count($optional_subjects)>0 && count(App\Models\OpttionalMark::where('school_id', $school->id)->where('year', $school->detail->running_session)->where('class_id', $class->id)->Where('exam_id', $exam->id)->where('class_id', $class->id)->where('student_id', $student->id)->get())>0)
		                            <br />
		                            <table class="table-left">
		                                <tr>
		                                    @foreach(App\Models\OpttionalMark::where('school_id', $school->id)->where('year', $school->detail->running_session)->where('class_id', $class->id)->Where('exam_id', $exam->id)->where('class_id', $class->id)->where('student_id', $student->id)->get() as $optional_subject)
		                                        <td>{{ $optional_subject->subject->name }}</td>
		                                    @endforeach
		                                </tr>
		                                <tr>
		                                    @foreach(App\Models\OpttionalMark::where('school_id', $school->id)->where('year', $school->detail->running_session)->where('class_id', $class->id)->Where('exam_id', $exam->id)->where('class_id', $class->id)->where('student_id', $student->id)->get() as $optional_subject)
		                                        <td>{{ $optional_subject->grade_obtained }}</td>
		                                    @endforeach
		                                </tr>
		                            </table>
		                        @endif 
